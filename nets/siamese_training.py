...
def create_img(char,url):
    font_list = os.listdir("font")

    font = ImageFont.truetype(font='font/' + font_list[0], size=85)
    size = [105, 105]
    new_image = Image.new('RGB', size, (255, 255, 255))
    label = '{}'.format(char)
    draw = ImageDraw.Draw(new_image)
    label = label.encode('utf-8')
    draw.text([10, 13], str(label, 'UTF-8'), fill=(0, 0, 0), font=font)
    new_image.save(url)
    # return new_image
class Generator:
    ...
    def generate(self,train=True):
        ...
        while 1:
            ...
            for index in  selected_characters_indexes :
                ...
                # size_dt=available_images[image_indexes[0]].split("_")[0]
                # size_dt_base= size_dt+"_"+"base.png"
                # base_url=os.path.join( image_path, size_dt_base)
                # if not os.path.exists(base_url):
                #     create_img(size_dt,base_url)
                ...
                image1 = os.path.join(image_path, available_images[image_indexes[1]])
                image_tem = os.path.join(image_path, available_images[image_indexes[0]])

                # print(rand_arr)
                # print("2：" + os.path.join(image_path, available_images[image_indexes[2]]))
                # 取出两张不类似的图片
                image = os.path.join(image_path, available_images[image_indexes[2]])
                arr = [image1, image_tem,image]
                # print(arr)
                rand_arr = np.random.choice(arr, 3, p=[ 0.3, 0.3,0.4], replace=False)
                batch_images_path.append(rand_arr[0])
                batch_images_path.append(rand_arr[1])
                batch_images_path.append(rand_arr[2])
                # 取出与当前的小类别不同的类
